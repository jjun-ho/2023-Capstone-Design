...
public:
     ...
     */
    KHomogeneous     ByTsai(const KMatrices& msA,   /**< 로봇운동으로 인한 카메라 프레임 사이의 변환행렬 리스트 */
     ...
     * @return F = H * M 를 만족하는 Homography 행렬 H
     */
    KMatrix                 Homography(const KPoints& psF,/**< 대응되는 특징점 리스트 A*/
                                       const KPoints& psM,/**< 대응되는 특징점 리스트 B*/
    ...
    static KHomogeneous     TransformFrom3D_3D(const KArray<KVector>& avA,
    ...
    static  KMatrix         TransformSimilarity(const KPoints& psFrom, const KPoints& psTo);
    static  KMatrix         TransformSimilarity(const std::vector<KPoint> lFrom, const std::vector<KPoint>& lTo);
