...
{
	if(KList<KPoint>::Count() != nNum)
	{
		if (_pItems)
			delete[] _pItems;
		_pItems = new KPoint[nNum];
	}

	for(int i=0; i<nNum; i++)
        _pItems[i]._dX = _pItems[i]._dY = 0.0;

	_uiCapacity = _uiCount = nNum;
}

KRect KPoints::Rect() const
{
    KRect rcOut(_pItems[0],_pItems[1]);

    for(int i=2; i<KList<KPoint>::Count(); i++)
    {
        rcOut._nLeft   = _MIN(rcOut._nLeft,_pItems[i]._dX);
        rcOut._nTop    = _MIN(rcOut._nTop,_pItems[i]._dY);
        rcOut._nRight  = _MAX(rcOut._nRight,_pItems[i]._dX);
        rcOut._nBottom = _MAX(rcOut._nBottom,_pItems[i]._dY);
    }

    return rcOut;
}

KPoints KPoints::Transform(const KMatrix& mHgp)
{    
    KPoints     psOut;
    KVector     vUV;    

    for(int i=0; i<KList<KPoint>::Count(); i++)
    {
        vUV = mHgp * KVector(_pItems[i]._dX, _pItems[i]._dY, 1.0);
        if(vUV[2] == 0.0) continue;
        vUV /= vUV[2];

        psOut.Add(KPoint(vUV[0],vUV[1]));
    }

    return psOut;
}

void KPoints::Transformed(const KMatrix& mHgp)
{
    KVector vUV;
...
{
	if (this->Count() != psInput.Count())
		Create(psInput.Count());
...
void KStrings::Create(const int& nNum)
{
    if(KList<KString>::Count() != nNum)
    {
        if (_pItems)
            delete[] _pItems;
        _pItems = new KString[nNum];
    }
...
bool KStrings::Read(FILE* fp)
{
    int nNum;

    if(fread(&nNum, sizeof(int), 1, fp) != 1)
        return false;

    Create(nNum);
    for(int i=0; i<nNum; i++)
        if(_pItems[i].Read(fp) == false)
            return false;
...
KMatrix KMatrix::Cut(const KList<short>& lRow, const KList<short>& lCol) const
{
    KMatrix mOut(lRow.Count(), lCol.Count());

    for(int i=0; i<lRow.Count(); i++)
        for(int j=0; j<lCol.Count(); j++)
            mOut._ppA[i][j] = _ppA[lRow[i]][lCol[j]];

    return mOut;
}

KMatrix KMatrix::Cut(int row, const KList<short>& lCol) const
{
    KMatrix mOut(1,lCol.Count());

    for(int i=0; i<lCol.Count(); i++)
        mOut._ppA[0][i] = _ppA[row][lCol[i]];

    return mOut;
}

KMatrix KMatrix::Cut(const KList<short>& lRow, int col) const
{
    KMatrix mOut(lRow.Count(),1);

    for(int i=0; i<lRow.Count(); i++)
        mOut._ppA[i][0] = _ppA[lRow[i]][col];
...
void KMatrices::Create(int nNum)
{
    ...
    KList<KMatrix>::_pItems = new KMatrix[nNum];

    KList<KMatrix>::_uiCapacity = KList<KMatrix>::_uiCount = nNum;
}
//---------------------------------------------------------------------------
...
KVector KVector::Cut(const KList<short>& lIdx) const
{
#ifdef _DEBUG
    if(lIdx.Max()+1 > Dim())
        throw("Exception in KVector::Cut(const KList<short>& lIdx)");
#endif

    KVector vOut(lIdx.Count());

    for(int i=0; i<lIdx.Count(); i++)
        vOut._ppA[i][0] = _ppA[lIdx[i]][0];

    return vOut;
}
//---------------------------------------------------------------------------

void KVector::Find(const double& dValue, KList<short>& lIndex) const
{
    lIndex.RemoveAll();
...
{    
	...
	KList<KVector>::_pItems = new KVector[nNum];

	KList<KVector>::_uiCapacity = KList<KVector>::_uiCount = nNum;
}
//---------------------------------------------------------------------------
...
void KImageDouble::Convolution(const KVector& vKernel,const int& nAxis, const bool& bExcludeBound,KImageDouble& idOut)
{
#ifdef _DEBUG
    ...
    nEx = (nAxis == _X_AXIS || bExcludeBound ? nCol-nHalf : 0);
    nSy = (nAxis == _Y_AXIS || bExcludeBound ? nHalf : 0);
    nEy = (nAxis == _Y_AXIS || bExcludeBound ? nRow-nHalf : 0);
...
void KImageDouble::Convolution(const double& dSigma,const int& nAxis, const bool& bExcludeBound,KImageDouble& idOut)
{
    KVector vKernel = KGaussianMulti::Kernel_1D(dSigma);

    Convolution(vKernel, nAxis, bExcludeBound, idOut);
}
...
KImageGray KImageGray::Thresholded_output(const int& nThresh, KImageGray& icThreshold)
{
    for(int i=0; i < Row(); i++){
        for(int j=0; j < Col(); j++){
            icThreshold[i][j] = (_ppA[i][j] > nThresh ? 255 : 0);
        }
    }
...
KBlobList* KImageGray::BlobColoring(int nType,KRect* rcpScan,KImageColor* icpColor)
{
	...
	for(int i=nSy+1,ii=nRow-1,ic=1; ii; i++,ic++,ii--)
	{
		...
		{
			uCur = _ppA[i][j];
            if(nType == _WHITE && uCur == 0)
				continue;
            else if(nType == _BLACK && uCur)
				continue;
...
KBINARIZATION_OUTPUT* KHisto::Otsu(const KListPixel* lpPixel)
{
#ifdef _DEBUG
...
void KHomogeneous::Create(const KVector& vRT, int nType)
{
    if(nType == _RODRIGUES)
        ...
        Place(0,0,KRotation(vRT[0],vRT[1],vRT[2], nType)); //_EULER : _RAD or _DEG
...
WThread::WThread(bool bExecuteNotify,unsigned int dwPriority,DWORD dwWaitExecute)
{
    ...
    _nWaitTerminate  = _MIN((int)(dwWaitExecute*1.2),2000);
}
