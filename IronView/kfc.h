...
typedef KArray<int>             KArrayInt;      //tag: an
...
public:
    ...
    unsigned int    Add(const KList<T>& lAdd);
    void            UpdateCapacity(const unsigned int& uiSize = 0);
    unsigned int    Find(const T&) const;
    void            Find(const T&, KList<T>& lIdx) const;
    ...
    T               Max(int* npIdx = 0) const;
    T               Min(int* npIdx = 0) const;
    ...
    void            Item(const KList<T>& lIndex, KList<T>& lItem) const;
...
typedef KList<KPIXEL>   KPixelList;
typedef KList<KPOINT>   KPolygon;

class KPoint;

template< class T > class KPtrList //tag : pl
{
protected:
    char            _szID[30];
    unsigned int   	_uiCapacity;
...
public:
    ...
    void            Swap(const unsigned int&,const unsigned int&);
    int             Find(const T&, const int& nNum=0);
    T               Item(const unsigned int& uiID){ return _pItems[uiID]; }
...
class KMatrix;
class KRect;
class KPoints : public KList<KPoint>
{

public:

    KPoints(int nNum=0) : KList<KPoint>((nNum==0 ? 100 : nNum)){ if(nNum) Create(nNum); }
...
public:
    ...
    bool    Intersect(const KRECT& rc)const {
                return (InRect(rc.nLeft, rc.nTop)                || InRect(rc.nLeft + rc.nWidth - 1, rc.nTop) ||
                        InRect(rc.nLeft, rc.nTop + rc.nHeight -1)|| InRect(rc.nLeft + rc.nWidth - 1, rc.nTop + rc.nHeight -1));
    }
    bool    Intersect(const KRect& rc) const
            {
                return (InRect(rc._nLeft,rc._nTop)   || InRect(rc._nRight,rc._nTop) ||
                        InRect(rc._nLeft,rc._nBottom)|| InRect(rc._nRight,rc._nBottom));
            }
...
class KStrings : public KList<KString>  //tag: ss
{

public:
    KStrings(const int& nNum=0) : KList<KString>((nNum ? nNum : 1000)){ if(nNum) Create(nNum); }
    virtual ~KStrings(){}

    void        Create(const int& nNum);
    int         Add(const char* szStr){     return KList<KString>::Add(KString(szStr)); }
    int         Add(const KString&  stStr){  return KList<KString>::Add(stStr); }
    int         Add(const KStrings& ssStr){  return KList<KString>::Add(ssStr); }
    void        Write(FILE* fp);
    bool        Read(FILE* fp);

    KString&	operator [](const int& nID){        return _pItems[nID]; }
    KString     operator [](const int& nID) const { return _pItems[nID]; }
};
...
//
    ...
    KMatrix     Cut(const KList<short>& lRow, const KList<short>& lCol) const;
    KMatrix     Cut(int row, const KList<short>& lCol) const;
    KMatrix     Cut(const KList<short>& lRow, int col) const;
...
class KMatrices : public KList<KMatrix>
{

public:
    KMatrices(int nNum = 0) : KList<KMatrix>((nNum == 0 ? 100 : nNum)) { KList<KMatrix>::_uiCount = nNum; }
...
public:
   ...
   //operations
    ...
    KVector     Cut(const KList<short>& lIdx) const;
    void        Find(const double& value, KVector& vIndex) const;
    void        Find(const double& value, KList<short>& lIndex) const;
    //int         Find(const double& value) const;
...
typedef KArrayObject<KVector> KArrayVector;
...
class KVectors : public KList<KVector>
{

public:
	KVectors(int nNum = 0) : KList<KVector>((nNum == 0 ? 100 : nNum)) { KList<KVector>::_uiCount = nNum; }
...
public:
    ...
    KImageGray   Thresholded_output(const int& nThresh, KImageGray& icThreshold);
...
public:
    ...
    void            Convolution(const double& dSigma,  const int& nAxis,const bool& bExcludeBound,KImageDouble& idOut);
    KImageDouble    Convolution(const double& dSigma,  const int& nAxis,const bool& bExcludeBound);
    void            Convolution(const KVector& vKernel,const int& nAxis,const bool& bExcludeBound,KImageDouble& idOut);
    KImageDouble    Convolution(const KVector& vKernel,const int& nAxis,const bool& bExcludeBound);
...
public:
    ...
    KBINARIZATION_OUTPUT*	Otsu(const KListPixel* lpPixel);
...
public:
    WThread(bool bExecuteNotify,unsigned int dwPriority=THREAD_PRIORITY_NORMAL,DWORD dwWaitExecute=300);
    virtual ~WThread();
...
template< class T > KList<T>::KList( const KList<T>& oList)
{
    _pItems = 0;
    Create(oList.Capacity());
...
template< class T > unsigned int KList<T>::Remove(const T& value)
{
    for(unsigned int i=0; i<_uiCount; i++)
        if(_pItems[i] == value)
        {
            for(unsigned int ii=i; ii<_uiCount-1; ii++)
                _pItems[ii] = _pItems[ii+1];
...
template< class T > unsigned int KList<T>::Find(const T& val) const
{
    for(unsigned int i=0; i<_uiCount; i++)
        if(_pItems[i] == val)
            return i;
...
template< class T > void KList<T>::Find(const T& val, KList<T>& lIdx) const
{
    lIdx.RemoveAll();
...
template< class T > void KList<T>::UpdateCapacity(const unsigned int& uiSize)
{
    _uiCapacity = (uiSize ? _MAX(uiSize, _uiCount) : _uiCount);
...
template< class T > void KList<T>::Item(const KList<T>& lIndex, KList<T>& lItem) const
{
    lItem.RemoveAll();

    for(int i=0; i<lIndex.Count(); i++)
        lItem.Add(_pItems[lIndex[i]]);
}

template< class T > T KList<T>::Max(int* npIdx) const
{
    T max = _pItems[0];

    for(unsigned int i=1; i<_uiCount; i++)
        if(_pItems[i] > max)
        {
            max = _pItems[i];
            if(npIdx)
...
template< class T > T KList<T>::Min(int* npIdx) const
{
    T min = _pItems[0];
...
template< class T > unsigned int KList<T>::Add(const T& t)
{
#ifdef _DEBUG
    assert(_pItems != 0);
#endif
...
template< class T > unsigned int KList<T>::Add(const KList<T>& lAdd)
{
#ifdef _DEBUG
    assert(_pItems);
#endif
...
{
    Create(oList._uiCapacity);

    _uiCount = oList._uiCount;

    for(unsigned int i = 0; i < _uiCount; i++)
        _pItems[i] = oList._pItems[i];
...
template< class T > int KPtrList<T>::Find(const T& p, const int& nNum) //첫 nNum 개에 대해서 적용
{
	//update the list
    int nCount = (nNum ? nNum : _uiCount);
...
template< class T > unsigned int KPtrList<T>::Add(const T& t)
{
	if(_uiCount == _uiCapacity){
		//alloc. a new memory block
...
template< class T > void KPtrList<T>::UpdateCapacity(const unsigned int& uiSize)
{
    _uiCapacity = (uiSize ? _MAX(uiSize, _uiCount) : _uiCount);
