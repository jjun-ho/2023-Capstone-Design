...
ImageForm::ImageForm(const QString& q_stFile, const QString& q_stID, MainFrame* owner) :  QWidget(0), ui(new Ui::ImageForm)
{
    ...
    //영상처리용 메모리와 공유 설정
    if(_q_bmpMain->format() == QImage::Format_Grayscale8 && _iwMain.Address() == 0)
...
void ImageForm::DrawPolygon(QRectF q_rcBound,QPolygonF* q_pPolygon, const QColor& q_Color, int nWidth)
{
    QPainter  painter;
    QPen      pen;
    ...
    painter.setPen(pen);   
    painter.drawPolyline(*q_pPolygon);
    painter.end();
}
